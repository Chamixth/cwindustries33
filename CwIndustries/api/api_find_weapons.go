...
// @Router      /FindWeapons [GET]


    func FindWeaponsApi(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse,error) {


objectid := event.QueryStringParameters["objectid"]
    inputObj, err := dao.DB_FindWeapons(objectid)
    if err != nil {
